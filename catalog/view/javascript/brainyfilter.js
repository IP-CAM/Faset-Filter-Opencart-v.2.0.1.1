...
if (typeof BrainyFilter === 'undefined') {
        ...
        partlyRefresh: function(){
            ...
            this.ajaxHandler = jQuery.ajax({
                ...
                success: function(res) {
                    ...
                    urlParams = newUrl.split('&');
                    var count = 0;

                    for(var i=0; i < urlParams.length; i++)
                    {
                        if(urlParams[i].indexOf('path')>-1)
                        {
                            count++;
                        }
                    }
                    ...
                    if(count > 1)
                    {
                        newUrl = newUrl.replace(new RegExp("path=[0-9][0-9]&"),'');
                    }
            ...
            if (BrainyFilter.ajaxEnabled && !BrainyFilter.redirectTo) {
                ...
                this.ajaxHandler = jQuery.ajax({
                    ...
                    success: function(res) {
                        ...
                        //Заменяю path='id категории' с предыдущего, не выбранный
                        urlParams = newUrl.split('&');
                        var count = 0;
                        ...
                        //Перезагружаю страницу, если выбран другой тип автомобиля
                        var urlParams = newUrl.split('&'),
                            currentPath = urlParams[1];

                        if(localStorage.href.indexOf(currentPath) == -1)
                        {
                            location.reload();
                        }
    ...
    //        var searchFilters = {};
            ...
            var category_id = $("select[name=bfp_" + id + "] option:selected").data('category_id'),
                category = 'path='+category_id+'&',
                path = params[1].substr(0,4);

            if (bfilter !== '') {
                category.replace(new RegExp("path=[0-9][0-9]"),'path='+category_id+'&');
                bfilter = category + 'bfilter=' + bfilter;
            }
        ...
        changeTotalNumbers: function(data) {
            ...
            $ctrls.css("display","none")//attr('disabled', 'disabled')
                .not('option')
